...
module.exports = function(stationId, current) {
  ...
  for (let s = 0; s < current.timetable.length; s++) {
    const correctStation = (current.timetable[s].station == stationId);

    if (!correctStation) continue;
    ...
    const staff = currentRosterDuties.filter((currentRosterDuties) => currentRosterDuties.dutyName == current.timetable[s].serviceId);
    let LE = '';
    let LEShift = '';
    let TM = '';
    let TMShift = '';
