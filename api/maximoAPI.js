...
module.exports = {
  maximoNISList: function(maximoTokens) {
    return new Promise(function(resolve, reject) {
      ...
      let cookies = ''; // = 'JSESSIONID=' + tokens.cookies[0].value;
      ...
      const postRequest = http.request(options, function(res) {
        ...
        res.on('end', function() {
          ...
          parseString(xml, function(err, result) {
            // eslint-disable-next-line max-len
            let timeUpdated = result['soapenv:Envelope']['soapenv:Body'][0]['GetUpdatedObjectsResponse'][0]['Update'][0]['UpdateContent'][0]['Content'][0]['div'][0]['table'][0]['tr'][2]['td'][0]['table'][0]['tr'][0]['td'][0]['div'][0]['_'];
            timeUpdated = moment(timeUpdated, 'DD.MM.YYYY hh:mm:ss');
            // eslint-disable-next-line max-len
